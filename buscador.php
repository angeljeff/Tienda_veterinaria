...
include("config/bd.php");
    ...
    ?>
<?php $_buscar=(isset($_POST['buscar']))?$_POST['buscar']:"";
switch($_buscar)  {
    case "busq":
        $_busc= $_POST['buscarr']; ?>
        <script>
...
<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
      <?php include("config/bd.php");
        ...
        $sql = "SELECT p.id_producto, p.nombre_producto, p.precio_pro, p.cantidad_disp, p.fecha_elaboracion, p.fecha_vencimiento, p.imagen_po, p.id_proveedores, p.id_sub_cat, s.nombre_sub_ca, c.nombre_cat, pro.nombre_pro FROM productos as p INNER JOIN sub_categorias as s ON p.id_sub_cat= s.id_sub_cat INNER JOIN categorias as c ON s.id_categoria=c.id_categoria INNER JOIN proveedores as pro ON p.id_proveedores=pro.id_proveedores where p.estado_producto=1 and p.nombre_producto LIKE LOWER ('%".$PRODUC."%') ";
        $consultar = $conexion->prepare($sql);
        $consultar->execute();
        $countw= $consultar->rowCount();
        if($countw >0) {
            $productt=$consultar->fetchAll();
